...
export default function AdminRawGoods() {
  ...
  const getRawGoodsData = (field, fieldData) => {
    try {
      const db = getDatabase(app);
      const rawGoodsRef = query(ref(db, "rawGoods"), orderByChild(`${field}`));
      // const empRef = query(ref(db, "employees"), orderByChild(`${field}`), equalTo(`${fieldData}`));
      get(rawGoodsRef).then((snapshot) => {
        var rawGood = [];

        snapshot.forEach((childSnapshot) => {
          rawGood.push(childSnapshot.val());
        });
        console.log(rawGood);
  ...
  const onSubmit = (data, { resetForm }) => {
    ...
    try {
      ...
      const addData = {
        id: rawGoodsData.length,
        rawGoodsName,
        Details,
        Image,
      };
      setRawGoodsData([...rawGoodsData, addData]);
    } catch (err) {}
  };
